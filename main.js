...
function updateTranslations() {
  ...
  currentLocationButton.textContent =
    translations.current_location_button || "Use Current Location";
...
async function getWeatherOrForecast(cityOrCoords, type = "weather") {
  ...
  if (typeof cityOrCoords === "string") {
    ...
    cityOrCoords.latitude &&
    cityOrCoords.longitude
  ) {
    const endpoint = type === "weather" ? "weather" : "forecast";
  ...
  if (data) {
    ...
    container.innerHTML = `<p>${
      translations.could_not_fetch || "Could not fetch data"
    }</p>`;
...
function displayForecast(data) {
  forecastContainer.innerHTML = `<h2>${
    translations.forecast || "Forecast"
  }</h2>`;
...
function showError(error) {
  weatherInfo.innerHTML = `<p>${translations.error || "Error"}: ${
    error.message
  }</p>`;
  weatherInfo.classList.add("show");
}
